...
| `docs/PROJECT.md` | 同步更新           | **暂未更新**（用户要求等后续可能的功能调整后再一并更新）        |
...
- **富文本评论（表情 / 图片 / 加粗等）** —— 用户 2026-09-14 试用后提出，明确「后面再改」。
  当前只支持纯文本（`maxLength=500`，后端 `CharField(500)`）。
  将来要做需要一并考虑：
  1. 前端引入编辑器（表情选择器 + 图片上传），并决定是否允许 Markdown
  2. 后端 `content` 字段改为 `TextField`，且**必须做 XSS 过滤**（存储型 XSS 风险），或存结构化 JSON 由前端安全渲染
  3. 图片需要接入 `resource` 模块的上传能力，并考虑存储与引用计数（评论删了图要不要清）
  4. 评论内容校验（`validate_add_comment_params`）的长度口径要从「字符数」改为「渲染后长度」或放宽上限
  5. 现有 `content` 是纯文本，迁移时要兼容历史数据
