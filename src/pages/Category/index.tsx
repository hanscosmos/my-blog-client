...
function findCategoryByAlias(tree: ArticleCategoryItemType[], alias: string): ArticleCategoryItemType | null {
  for (const node of tree) {
    if (node.alias === alias) return node;
    if (node.children) {
      const found = findCategoryByAlias(node.children, alias);
...
export default function CategoryPage() {
  ...
  const categoryCacheRef = useRef<ArticleCategoryItemType[] | null>(null);

  const loadCategoryTree = useCallback(async () => {
    if (categoryCacheRef.current) return categoryCacheRef.current;
    const res = await getAllCategoryTreeApi();
    categoryCacheRef.current = res;
    return res;
  }, []);
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      ...
      const category = findCategoryByAlias(tree, alias || "");
      const categoryId = category?.id || "";
      setCategoryName(category?.name || alias || "");
